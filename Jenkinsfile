...
def runnerImage = "maxross/infra-runner:latest"
def playbookDir
...
pipeline {
  ...
  parameters {
      ...
      string(name: 'ANSIBLE_VARS', defaultValue: '', description: 'Additional variables to pass to Ansible in key=value format, separated by spaces')
  }
          ...
          property.useWithCredentials(['AWS_SSH_PEM_KEY_NAME', 'AWS_SSH_PEM_KEY', 'AWS_SSH_RSA_KEY_NAME', 'AWS_SSH_RSA_KEY']) {
            dir(sshDir) {
              if (env.SSH_KEY_TYPE == 'pem') {
                ...
                error "Unsupported SSH_KEY_TYPE: ${env.SSH_KEY_TYPE}. Supported types are 'pem' and 'rsa'."
              }
            }
          }
          property.useWithProperties(['AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY']) {
            dir("${repoRoot}/${TOFU_MODULE}") {
...
"""
      ...
      steps {
        script {
          container.runCommand(
            ...
            command: "sh -c \"tofu -chdir=${TOFU_MODULE} init && tofu -chdir=${TOFU_MODULE} apply -auto-approve\"",
            workingDir: "/tofu"
          )
        }
      }
    ...
    stage('Configure Ansible Inventory') {
      environment {
        TERRAFORM_NODE_SOURCE="${TOFU_MODULE}"
      ...
      steps {
        script {
          ...
          dir(repoRoot) {
            ...
            fqdn = tofu.getOutputs(dir: "${TOFU_MODULE}", output: 'fqdn')
            kube_api_host = tofu.getOutputs(dir: "${TOFU_MODULE}", output: 'kube_api_host')
          }
          ...
          container.runCommand(
            ...
            image: runnerImage,
            volumes: ["${repoRoot}:/tofu", "${sshDir}:/.ssh"],
            envVars: [TERRAFORM_NODE_SOURCE: "${TOFU_MODULE}"],
