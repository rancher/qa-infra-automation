...
| `ENABLE_UI_PLUGIN_MIRROR` | `no` | `yes`, `no` | Airgap opt-in: include the standalone `ui-plugin-mirror` step in `all`/`setup-from-infra`/`airgap-downstream` (stands up the `ui-plugin-charts` HTTP mirror on the bastion, always ordered before the Rancher deploy) **and** inject `enable_ui_plugin_mirror=true` into every Ansible invocation of the run (including `downstream`/`airgap-downstream`, which have no standalone mirror step) so the registration stage injects the bastion mirror URL/branch into `neuvectorTest` in `cattle-config.yaml`. An explicit `enable_ui_plugin_mirror=...` in `EXTRA_VARS` still wins (later `--extra-vars` keys override earlier ones). Ignored unless `ENV=airgap` |
| `ENABLE_CHARTS_MIRROR` | `no` | `yes`, `no` | Airgap (RKE2 only — k3s has no airgap env) opt-in: include the standalone `charts-mirror` step in `all`/`setup-from-infra` (stands up the `rancher-charts` git smart-HTTP mirror on the bastion, including a settled ref one publish behind the branch head, and the Rancher deploy repoints the `rancher-charts` ClusterRepo at it) **and** inject `enable_charts_mirror=true` into every Ansible invocation of the run (including `downstream`/`airgap-downstream`, so the downstream catalog repoint fires without extra flags). An explicit `enable_charts_mirror=...` in `EXTRA_VARS` still wins (later `--extra-vars` keys override earlier ones). Ignored unless `ENV=airgap` |
