...
Generate Rancher API tokens with configurable options for automation and CI/CD pipelines.
...
- **Token Scoping**: Create admin tokens or cluster-scoped tokens
...
| Variable | Default | Description |
|----------|---------|-------------|
| `rancher_token_description` | `automation-token-{timestamp}` | Token description for identification |
| `rancher_token_ttl` | `0` | Token TTL in milliseconds (0 = no expiration) |
| `rancher_token_cluster_id` | `""` | Cluster ID for scoped token (empty = admin token) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `rancher_set_permanent_password` | `false` | Set permanent password after first login |
| `rancher_permanent_password` | Same as token password | New permanent password |
...
| `rancher_api_retries` | `10` | Number of retries for API calls |
| `rancher_api_retry_delay` | `15` | Delay between retries (seconds) |
  ...
  roles:
    - role: rancher_token
      vars:
        ...
        rancher_token_description: "ci-pipeline-token"
```
  ...
  roles:
    - role: rancher_token
      vars:
        ...
        rancher_set_permanent_password: true
  ...
  roles:
    - role: rancher_token
      vars:
        rancher_url: "https://rancher.example.com"
        rancher_token_password: "mypassword"
        rancher_token_cluster_id: "c-m-abc123"
        rancher_token_description: "downstream-cluster-token"
```
...
```hcl
# Generated by rancher_token role at 2025-01-23T10:00:00Z
...
stage('Generate Admin Token') {
    sh """
        ansible-playbook playbooks/setup/generate-admin-token.yml \\
            -e rancher_url=https://${rancherHostname} \\
