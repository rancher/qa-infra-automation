...
| Field | Meaning |
| --- | --- |
| `kind` | `metadata`, `prime`, `prime-head`, `prime-ga`, `community-ga` or `community`. Decides how the child resolves its channel |
| `tag` | A branch head (`head`, `v2.14-head`) for `metadata` and `community`; a Rancher minor (`v2.14`) for `prime` and `prime-head`; a minor or a full version (`v2.14.5`) for the two GA kinds |
...
| `channel_source` | `metadata`, `prime`, `prime-head`, `prime-ga`, `community-ga` or `fixed` | always |
