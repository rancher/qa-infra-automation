...
### `prime-head`, the in-flight build of a minor

`prime:` falls back to a head build only when a minor has no release. That
leaves the common case unreachable: development resumes on a branch the moment
a patch ships, so every maintained minor has a head build ahead of its newest
release, and `prime:` will never pick it.

| Minor | `prime:` | `prime-head:` |
| --- | --- | --- |
| 2.13 | `2.13.9` | `2.13.10-<sha>-head` |
| 2.14 | `2.14.5` | `2.14.6-<sha>-head` |
| 2.15 | `2.15.1` | `2.15.2-<sha>-head` |
| 2.16 | `2.16.0-<sha>-head` | `2.16.0-<sha>-head` |

`prime-head:` skips the release pick and takes the head directly. The two kinds
share one code path: `prime:` sets `_prime_pick` from releases first and the
head task fires only when that found nothing, while `prime-head:` never runs the
release pick, so the same task fires every time.

They agree on the newest minor, because a minor with no release has nothing else
to pick. That is the same answer for a different reason, not a special case.

Use `prime-head:` to test what is in flight, before an rc opens. It is
deliberately **not** reproducible: the row resolves to a different commit on
every run, which is why `prime:` cannot behave this way and why a result worth
keeping has to record the resolved version rather than the row. The staleness
check still applies, so a minor whose head builds have stopped fails rather than
testing frozen software.

Do not reach for it to pin one build. An exact chart version on the `fixed` path
does that and stays reproducible:

```yaml
channel_source: "fixed"
rancher_helm_repo: "rancher-latest"
rancher_image_tag: "2.13.10-123c32c-head"
```
