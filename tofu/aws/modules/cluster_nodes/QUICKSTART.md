...
aws_vpc               = "vpc-" # Optional: fill in a pre-existing VPC, or omit to auto-create an ephemeral one
aws_subnet            = "subnet-" # Optional: fill in a pre-existing subnet, or omit to auto-create an ephemeral one
aws_security_group    = ["sg-"] # Optional: fill in pre-existing security group ID(s), or omit ([]) to auto-create an ephemeral one
airgap_setup          = false
proxy_setup           = false
aws_volume_size       = 40
aws_volume_type       = "gp3"
